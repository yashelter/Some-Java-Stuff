...
@Slf4j
public final class KafkaFilterController implements KafkaReader {
    ...
    public static KafkaFilterController createNewInstance(Config config) {
        ...
        RuleProcessor processor = new RulesController();
    ...
    public void processing() {
        try {
            ...
            for(;;) {
                ...
                log.error("!MESSAGES: {}!", records.count());
                ...
                for (ConsumerRecord<String, String> recording : records) {
                    Message message = ruleProcessor.processing(Message.builder()
                            .value(recording.value()).filterState(false).build(), rules);
                    writer.processing(ruleProcessor.processing(message, rules));
                }
                commitChanges();
            }
